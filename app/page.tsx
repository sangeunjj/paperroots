  ...
  async function run() {
    ...
    try {
      const res = await fetch("/api/research", {
        ...
        body: JSON.stringify({ topic, role, extraKeywords: extra, yearRange, openAccessOnly, reviewFocus }),
      });
      const json = await res.json();
      if (!res.ok) setError(json.error || "오류가 발생했습니다.");
      else setData(json as ResearchResponse);
    } catch {
  ...
  return (
    <div className="wrap">
      ...
      <section className="search">
        ...
        <div className="block">
          <p className="label">관심 주제 <span className="hint">(영어 키워드 권장)</span></p>
          <input
            className="field"
            value={topic}
        ...
        <div className="block">
          <p className="label">세부 관심사 <span className="hint">(선택 · 주제를 좁힐 키워드)</span></p>
          <input
            className="field"
            value={extra}
            onChange={(e) => setExtra(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && run()}
            placeholder="예: agent, evaluation, pretraining, medical …"
          />
        </div>

        <div className="block">
          <p className="label">검색 기간</p>
          <div className="chips">
            {YEARS.map((y) => (
              <button
                key={y.value}
                className={`chip ${yearRange === y.value ? "active" : ""}`}
        ...
        <div className="block">
          <p className="label">추가 옵션</p>
          <div className="chips">
            <button
              className={`chip toggle ${reviewFocus ? "active" : ""}`}
              onClick={() => setReviewFocus((v) => !v)}
              type="button"
            >
              📚 리뷰·서베이 중심
            </button>
            <button
              className={`chip toggle ${openAccessOnly ? "active" : ""}`}
      ...
      {data && !loading && (
        <div className="results">
          ...
          <p className="meta">
            ...
            {(data.meta.elapsedMs / 1000).toFixed(1)}초
          </p>
      ...
      <footer className="footer">
        데이터: <code>OpenAlex</code> · 요약: <code>Claude</code> · 만든 도구:{" "}
